...
if check_password():
    ...
    uploaded_files = st.file_uploader("", type=["avi", "mp4", "mov"], accept_multiple_files=True)
    st.markdown("<hr>", unsafe_allow_html=True)

    if st.button("Retranscrire les vidéos en texte"):
        if uploaded_files:
            try:
                ...
                for index, uploaded_file in enumerate(uploaded_files):
                    ...
                    try:
                        ...
                        audio_output_path = f"/tmp/{Path(uploaded_file.name).stem}.mp3"
                        video.audio.write_audiofile(audio_output_path)
                    except Exception as e:
                        st.error(f"Erreur lors de l'extraction de l'audio de la vidéo {uploaded_file.name} : {str(e)}")
                        continue

                    try:
                        # Load the extracted MP3 file
                        audio = AudioSegment.from_mp3(audio_output_path)
                        duration = len(audio)
                        ten_minutes = 10 * 60 * 1000  # 10 minutes in milliseconds
                    except Exception as e:
                        st.error(f"Erreur lors du chargement de l'audio de {uploaded_file.name} : {str(e)}")
                        continue
                    ...
                    try:
                        ...
                        st.error(f"Erreur lors de la transcription de la vidéo {uploaded_file.name} : {str(e)}")
                        continue
            ...
            st.error("Veuillez téléverser au moins 1 fichier vidéo")
